 ...
 * FUNCTIONS
 */
static gboolean ly_3vdo_on_expose_cb(GtkWidget *widget, cairo_t *cr, gpointer data);
void ly_3vdo_on_meta_changed_cb(LyMbsMessage *message, gpointer data);
gboolean ly_3vdo_on_get_list_cb(gpointer stmt, gpointer data);
gboolean ly_3vdo_on_list_active_cb(GtkTreeView *view, GtkTreePath *path, GtkTreeViewColumn *column,gpointer data);
gboolean ly_3vdo_on_list_add_cb(GtkWidget *widget, gpointer data);
...
GtkWidget *ly_3vdo_create()
{
	...
	g_signal_connect(G_OBJECT(view), "row-activated", G_CALLBACK(ly_3vdo_on_list_active_cb), NULL);
...
{
	ly_pqm_clear_md();
	ly_dbm_exec("UPDATE metadatas SET playing=num WHERE flag>=10 AND flag<=19", NULL, NULL);
...
gboolean ly_3vdo_on_list_refresh_cb(GtkWidget *widget, gpointer data)
{
	...
	ly_3vdo_list_store = gtk_list_store_new (3,G_TYPE_INT,G_TYPE_STRING,G_TYPE_STRING);
	ly_dbm_exec("SELECT id, title, duration FROM metadatas WHERE flag>=10 AND flag<=19 ORDER BY num", ly_3vdo_on_get_list_cb, NULL);
	gtk_tree_view_set_model(GTK_TREE_VIEW (ly_3vdo_list_view), GTK_TREE_MODEL(ly_3vdo_list_store));
	return FALSE;
}
...
gboolean ly_3vdo_on_list_del_cb(GtkWidget *widget, gpointer data)
{
	...
	list=gtk_tree_selection_get_selected_rows(GTK_TREE_SELECTION(ly_3vdo_list_selection),NULL);
	if(list==NULL)
		return FALSE;
	...
	while(p)
	{
		gtk_tree_model_get_iter(GTK_TREE_MODEL(ly_3vdo_list_store), &iter, (GtkTreePath *)(p->data));
		gtk_tree_model_get(GTK_TREE_MODEL(ly_3vdo_list_store), &iter, 0, &id, -1);
		if(p==list)
		{
