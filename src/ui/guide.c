...
gboolean ly_ui_guide_on_audio_button_clicked_cb(GtkWidget *widget, gpointer data);
gboolean ly_ui_guide_on_lib_entry_changed_cb(GtkWidget *widget, gpointer data);
gboolean ly_ui_guide_on_lib_open_clicked_cb(GtkWidget *widget, gpointer data);
gboolean ly_ui_guide_on_lib_load_clicked_cb(GtkWidget *widget, gpointer data);
gpointer ly_ui_guide_on_load_lib_cb(void *data);

gboolean ly_ui_guide_init()
{
	if(LY_GLOBAL_FIRST_FLAG==FALSE)
		return TRUE;
	...
	g_snprintf(path, sizeof(path), "%sui/icon/wizard.svg", LY_GLOBAL_PROGDIR);
	...
	g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(ly_ui_guide_on_audio_button_clicked_cb), page);
	...
	entry=gtk_entry_new();
	g_snprintf(text, sizeof(text), "%smusic/", LY_GLOBAL_HOMEDIR);
	if(!ly_conf_get("lib_path", "%s", text))
	{
		ly_conf_set("lib_path", "%s", text);
	}
	gtk_entry_set_text(GTK_ENTRY(entry), text);
	g_signal_connect(G_OBJECT(entry),"changed",G_CALLBACK(ly_ui_guide_on_lib_entry_changed_cb),NULL);
	gtk_box_pack_start(GTK_BOX(hbox),entry,TRUE,TRUE,0);
	button=gtk_button_new_from_stock(GTK_STOCK_OPEN);
	g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(ly_ui_guide_on_lib_open_clicked_cb),entry);
	gtk_box_pack_start(GTK_BOX(hbox),button,FALSE,FALSE,0);
	button=gtk_button_new_with_label(_("Load"));
	gtk_box_pack_start(GTK_BOX(hbox),button,FALSE,FALSE,0);
	bar=gtk_progress_bar_new();
	gtk_orientable_set_orientation(GTK_ORIENTABLE(bar), GTK_ORIENTATION_HORIZONTAL);
	g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(ly_ui_guide_on_lib_load_clicked_cb),bar);
...
gboolean ly_ui_guide_on_audio_button_clicked_cb(GtkWidget *widget, gpointer data)
{
	if(ly_audio_get_state()==GST_STATE_PLAYING)
	{
		ly_audio_stop();
	...
	{
		char path[1024];
		g_snprintf(path,sizeof(path), "file://%sui/audio/test.ogg", LY_GLOBAL_PROGDIR);
		if(ly_audio_meta)
		{
			g_free(ly_audio_meta);
		}
		ly_audio_meta=ly_db_read_metadata(path);
		ly_audio_play();
...
gboolean ly_ui_guide_on_lib_entry_changed_cb(GtkWidget *widget, gpointer data)
{
	ly_conf_set("lib_path", "%s", gtk_entry_get_text(GTK_ENTRY(widget)));
	return FALSE;
}
...
gboolean ly_ui_guide_on_lib_load_clicked_cb(GtkWidget *widget, gpointer data)
{
	GThread *thread=g_thread_create(ly_ui_guide_on_load_lib_cb, data, FALSE, NULL);
	return FALSE;
}
...
gpointer ly_ui_guide_on_load_lib_cb(void *data)
{
	 ...
	 */
	GList *list=NULL;
	list=ly_global_traverse_dir(path, 5, TRUE);
	if(list==NULL)
		return FALSE;
	GList *p=list;
	gchar *sql=NULL;
