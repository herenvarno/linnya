...
gboolean ly_key_read(void)
{
	...
	{
		ly_msg_put("warning", "ui:key", _("Key defination file does not exist, linnya will create a new one when exit!"));
		return TRUE;
	}
	...
	{
		ly_msg_put("warning", "ui:key", _("Configuration file error!"));
...
gboolean ly_key_write(void)
{
	...
	if(!(fp=fopen(path,"w+")))
	{
		ly_log_put(_("[warning] Cannot write keybinds to file!"));
		return FALSE;
	}
